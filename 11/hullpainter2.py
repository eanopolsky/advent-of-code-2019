...
robot = {"x": 0,
         "y": 0,
         "facing": "up"} #valid values: up, down, left, right
lastaction = "none"

def getcurrentcolor():
    try:
        return hullgrid[(robot["x"],robot["y"])]
    except KeyError:
        return 0 #hull is black by default

def handleoutput(vmoutput):
    global lastaction
    if lastaction == "none" or lastaction == "move":
        #time to paint
        hullgrid[(robot["x"],robot["y"])] = vmoutput
        lastaction = "paint"
        return
    elif lastaction == "paint":
        #time to move
        if vmoutput == 0: #turn left
            leftturn = {"up": "left",
                        "left": "down",
                        "down": "right",
                        "right": "up"}
            robot["facing"] = leftturn[robot["facing"]]
        elif vmoutput == 1: #turn right
            rightturn = {"up": "right",
                         "right": "down",
                         "down": "left",
                         "left": "up"}
            robot["facing"] = rightturn[robot["facing"]]
        else:
            print("unsupported robot command")
            exit(1)
        #move forward
        if robot["facing"] == "up":
            robot["y"] += 1
        elif robot["facing"] == "down":
            robot["y"] -= 1
        elif robot["facing"] == "left":
            robot["x"] -= 1
        elif robot["facing"] == "right":
            robot["x"] += 1
        else:
            print("unsupported robot direction")
            exit(1)
        lastaction = "move"
    else:
        print("unsupported robot last action")
        exit(1)
            
def renderhullgrid(hullgrid):
    xoords = [panel[0] for panel in hullgrid]
    yoords = [panel[1] for panel in hullgrid]
    minx = min(xoords)
    maxx = max(xoords)
    miny = min(yoords)
    maxy = max(yoords)
...
if __name__ == "__main__":
    ...
    ehp = intcodevm(memory=memory,name="emergency-hull-painter")
    ehp.setinputfunc(getcurrentcolor)
    ehp.setoutputfunc(handleoutput)
    hullgrid[(0,0)] = 1
    ehp.run()
    renderhullgrid(hullgrid)
