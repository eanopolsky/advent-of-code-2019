...
def tracewirelist2point(wirelist,targetpoint):
    points = []
    currentpoint = [0,0]
    points.append(currentpoint)
    totaldistance = 0
    ...
    print("wirelist does not reach point")
    exit(1)

def listintersection(list1,list2):
    #return [value for value in list1 if value in list2]
    # ^ too slow
    listoftuples1 = [tuple(point) for point in list1]
    listoftuples2 = [tuple(point) for point in list2]
    intersectiontuples = set(listoftuples1) & set(listoftuples2)
    intersectionlist = [list(point) for point in intersectiontuples]
    return intersectionlist

def findleastmanhattandistance(pointlist):
    for i in range(len(pointlist)):
        pointlist[i].append(abs(pointlist[i][0])+abs(pointlist[i][1]))
    #print(pointlist)
    pointlist.sort(key=lambda point: point[2])
    print(pointlist[1])

def findclosestintersection(wire1,wire2):
    print("Generating points lists.")
    wire1points = wirelist2points(wire1)
    wire2points = wirelist2points(wire2)
    print("finding intersections")
    print(len(wire1points))
    print(len(wire2points))
    intersectionpoints = listintersection(wire1points,wire2points)
    print(len(intersectionpoints))
    #print("computing manhattan distances")
    #findleastmanhattandistance(intersectionpoints)
    print("sorting by total length")
    intersectionpoints.remove([0,0])
    intersectionpoints.sort(key=lambda ipoint: tracewirelist2point(wire1,ipoint)+tracewirelist2point(wire2,ipoint))
    # for ipoint in intersectionpoints:
    #     print("tracing to point {}".format(ipoint))
    #     print(str(tracewirelist2point(wire1,ipoint)+tracewirelist2point(wire2,ipoint)))
    print(intersectionpoints[0])
    print(tracewirelist2point(wire1,intersectionpoints[0])+tracewirelist2point(wire2,intersectionpoints[0]))



findclosestintersection(wire1,wire2)
